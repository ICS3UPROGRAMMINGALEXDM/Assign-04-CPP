...
string uString, finalString, newString, uSize, uCopies, answer = "y", charArr;
int arryLength;
float copies, size;
...
void looping() {
	...
	finalString = "";
	newString = "";
	// inner loop is meant to meet the size requirements, outer loop is for copies
...
int main() {
	do {
        ...
        while (errorLoop){
        	// get size
        	cout << BOLDWHITE << "Enter how many characters long you want the Splice&Diced word to be: ";
        	cin >> uSize;
        	...
        	try{
        		...
        		// also checks to make sure that no extra decimal spaces were entered
				if (
					size < 1 || copies < 1 ||
					size > uString.length() ||
					copies != static_cast<int>(copies) ||
					size != static_cast<int>(size)) {
					cout << BOLDMAGENTA << "Make sure copies and size are greater than 0. AND that " <<
					...
					looping();
					while (answerLoop) {
						cout << BOLDWHITE << "Would you like to Splice&Dice another word? (y/n): ";
						cin >> answer;
						...
						if (answer == "n") {
							...
							cout << "I'm not sure I understand. Please try again." << endl;
						}
					}
				}
        	} catch (...) {
        		cout << BOLDRED <<
        		"Please ensure size and copies are vaild and positive integers" <<
        		BOLDWHITE << endl;
        	}
        }
	} while(answer == "y");
